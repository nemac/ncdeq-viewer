...
function AGO_AllChartData_byID(hucid, current_geography_level){
   ...
   const query_URL = '/' + FEATURE_SERVICE_NAME + '/FeatureServer/' + NCDEQ_NORMALIZED_FEATUREID + '/query' +
                   '?where=chart_value+is+not+null+and+ID+like+%27' + id + '%25%27+and+geography_level%3D'+ next_level +
                   '&objectIds=' +
                   '&time=' +
                   '&resultType=none' +
...
function ago_getPreviousChart(chart_level, chart_id){
     ...
     const query_URL = '/' + FEATURE_SERVICE_NAME + '/FeatureServer/' + NCDEQ_NORMALIZED_FEATUREID + '/query' +
                          '?where=chart_value+is+not+null+and+chart_level+%3D+' + chart_level + '+and+chart_id+%3D+' + chart_id  + 
                          '&objectIds=' +
                          '&time=' +
                          '&resultType=none' +
                          '&outFields=chart_level%2C+chart_matchid%2C+chart_level_label%2C+chart_type%2C+chart_id' +
